...
export type BucketEventHandler = aws.lambda.EventHandler<BucketEvent, void>;
...
let bucketSubscriptionInfos = new Map<aws.s3.Bucket, SubscriptionInfo[]>();
 ...
 */
export class BucketEventSubscription extends aws.lambda.EventSubscription {
  ...
  public constructor(
    ...
    this.func = aws.lambda.createFunctionFromEventHandler(
      name,
      handler,
      parentOpts,
    );
...
process.on('beforeExit', () => {
  ...
  // Make sure to clear out the pending subscriptions array so that we don't try to apply them again.
  ...
  for (const [bucket, subscriptions] of copy) {
    ...
    const opts = {
      ...
      provider: subscriptions[0].provider,
    };
...
aws.s3.Bucket.prototype.onObjectCreated = function (
  ...
  const argsCopy: BucketEventSubscriptionArgs = {
    filterPrefix: args.filterPrefix,
    filterSuffix: args.filterSuffix,
...
aws.s3.Bucket.prototype.onObjectRemoved = function (
  this: aws.s3.Bucket,
  name,
  handler,
  args,
...
aws.s3.Bucket.prototype.onEvent = function (
  this: aws.s3.Bucket,
  name,
  handler,
  args,
